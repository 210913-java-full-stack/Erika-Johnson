...
###########################################################
DROP DATABASE IF EXISTS Erika_db;
CREATE DATABASE Erika_db;
...
SELECT SUM(balance) AS "Smith Total Balance"
FROM customers c 
JOIN accounts_customers ac ON c.customer_id = ac.customer_id 
JOIN accounts a ON ac.account_id = a.account_id
WHERE name LIKE '%Smith';
...
SELECT c.name , ad.address , ad.city , ad.state , a.balance 
FROM customers c 
JOIN accounts_customers ac ON c.customer_id = ac.customer_id
JOIN accounts a ON ac.account_id = a.account_id
JOIN address ad ON c.address_id = ad.address_id
...
JOIN address ad ON ad.address_id = ad.address_id 
SET balance = (a.balance +100)
WHERE ad.state = 'NY';

# Transfer $199.99 from Jason Smith to Amanda Smith(Substract amount from one account and add to other,need two statements)

UPDATE accounts a 
JOIN accounts_customers ac ON a.account_id = ac.account_id 
JOIN customers c ON ac.customer_id = c.customer_id 
SET balance = (a.balance - 199.99)
